...
// probably want to use BestReadyBlock.
...
// BestReadyBlock retrieves the most recent block's height and hash where we
// have both the header and filter header ready.
func (s *ChainService) BestReadyBlock() (*waddrmgr.BlockStamp, error) {
	bestHeader, bestHeight, err := s.BlockHeaders.ChainTip()
	if err != nil {
		return nil, err
	}
