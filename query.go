...
func (s *ChainService) GetCFilter(blockHash chainhash.Hash,
	...
	msgs := []*query.Query{
		&query.Query{
			...
			HandleResp: func(_, resp wire.Message, _ string) (bool, bool) {
				return s.handleCFiltersResponse(cfQuery, resp)
			},
		},
	}

	// Translate options to query.QueryOption type, and hand the queries to
	// the work manager.
	errChan := s.queryAccess.Query(
...
// translateQueryOptions to query.QueryOption type.
...
func (s *ChainService) GetBlock(blockHash chainhash.Hash,
	...
	q := &query.Query{
		Req: getData,
		HandleResp: func(_, resp wire.Message, peerAddr string) (bool, bool) {
